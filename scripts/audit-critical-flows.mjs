...
expect("src/lib/ml-discovery.server.ts", ["/sites/MLB/search", "strict_verified_marketplace_items", "marketplaceApiSearch", "verifyCandidates", "verified_item === true", 'item.status === "active"', "isMercadoLivrePermalink", "discoverMlItemLinksWithGoogle", 'source_kind: "marketplace"'], "busca oficial exibe somente anúncios reais e validados do marketplace");
expectNot("src/lib/ml-discovery.server.ts", ["/products/search?status=active&site_id=MLB&q=", "baseItem("], "busca oficial não usa catálogo nem candidatos não validados");
...
expect("src/lib/ml-public-search.functions.ts", ["official-and-public-parallel-verify-every-result", "const officialPromise = tokensPromise.then", "const fallbackPromise = searchMercadoLivrePublicSiteFallback", "Promise.all([tokensPromise, officialPromise, fallbackPromise])", "verifyCandidates", "item.verified_item === true", "itemIdFromRealMlUrl(item.permalink) === item.id", "firecrawlEnrichMercadoLivreAds", "searchAdsWithGeminiGrounding"], "endpoint comum roda fontes iniciais em paralelo, enriquece dados e só exibe itens confirmados");
expectNot("src/lib/ml-public-search.functions.ts", ["addItems(byId, groundedCandidates)", "marketplace-keyword-multi-offer"], "grounding não entra direto na lista e busca não usa contrato antigo");
expect("src/lib/ml-search-production.functions.ts", ["isConfirmedRealMlItem", "itemIdFromRealMlUrl(item.permalink) === id", "verified_item: verifiedItem", "isConfirmedActiveMlItem", "mapped.filter(isConfirmedActiveMlItem)", "result.item && isConfirmedRealMlItem(result.item)"], "buscas por produto, vendedor, ID e link também exigem anúncio real confirmado");
expect("src/lib/ml-public-site-fallback.server.ts", ["https://lista.mercadolivre.com.br/", "normalizeSearchTerm(query).toLowerCase()", "itemIdFromRealMlUrl", "extractPriceCents", "extractSeller", "extractStructuredItems", 'source_kind: isCatalogOffer(block) ? "catalog_offer" : "marketplace"'], "fallback público extrai dados somente de anúncios com URL real do Mercado Livre e tolera mudanças de HTML");
expect("src/lib/ml-firecrawl-enrich.server.ts", ["url: candidate.permalink", "itemIdFromRealMlUrl(candidate.permalink) !== candidate.id", "extractProductPageEnrichment", "og:image", "itemprop"], "Firecrawl visita o permalink real somente para completar dados faltantes");
expect("src/routes/_authenticated/buscar.tsx", ["createListingDraft", "verified_item", "Comparar no ML", "normalizeSearchTerm", "Preço, vendas, estoque e situação só aparecem quando há confirmação", "Ver anúncio", "Duplicar anúncio", "Duplicação criada no painel", 'to: "/editor/$id"'], "busca/cópia mostra métricas verificadas, ações claras e confirmação real da duplicação");
...
expect("src/lib/publish.functions.ts", ["published_ml_id", "Não publique novamente", "publishListingToMlWithStatus", 'z.enum(["active", "paused"])', "status: result.status"], "publicação idempotente aceita modo ativo ou pausado");
expect("src/lib/ml-publish-mode.server.ts", ['body: JSON.stringify({ status: "paused" })', "nunca repetimos o POST /items", 'status: "active"'], "publicação pausada não repete criação quando a etapa de pausa falha");
expect("src/components/app/PublishButton.tsx", ["Publicar já ativo", "Publicar como pausado", "Publicado pausado no Mercado Livre", "Publicado ativo no Mercado Livre"], "interface permite escolher o estado inicial da publicação");
