...
/** Ativação manual de licença (chave entregue por Pix/WhatsApp, cortesia, etc.). */
export const activateLicense = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (license.status === "cancelled" || license.status === "suspended") {
      return { ok: false as const, reason: "Esta licença está suspensa ou cancelada." };
    }
    if (license.expires_at && new Date(license.expires_at) < new Date()) {
      return { ok: false as const, reason: "Esta licença está expirada." };
    }
    if (license.user_id && license.user_id !== context.userId) {
      return { ok: false as const, reason: "Esta licença já está vinculada a outra conta." };
    }

    const months = PERIOD_MONTHS[license.period] ?? 1;
    const startsAt = license.starts_at ?? new Date().toISOString();
    const expiresAt =
      license.expires_at ??
      new Date(new Date(startsAt).setMonth(new Date(startsAt).getMonth() + months)).toISOString();
...
/** Geração de licenças — exclusiva do administrador. */
export const generateLicenses = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const months = PERIOD_MONTHS[data.period] ?? 1;
    ...
    for (let i = 0; i < data.quantity; i += 1) {
      ...
      rows.push({
        ...
        period: data.period,
        origin: data.origin,
        status: data.user_id ? ("active" as const) : ("available" as const),
        user_id: data.user_id ?? null,
        activated_at: data.user_id ? new Date().toISOString() : null,
        starts_at: startsAt,
        expires_at: expiresAt,
        note: data.note ?? null,
