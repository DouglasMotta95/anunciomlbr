...
async function mapItemRaw(raw: Record<string,unknown>, tokens: string[], source_kind: SearchMlItem["source_kind"]="marketplace"): Promise<SearchMlItem> {
  ...
  const seller = typeof sellerObj?.nickname === "string" ? sellerObj.nickname : await sellerNickname(sellerId ?? (sellerObj?.id != null ? String(sellerObj.id) : null), tokens);
  return {
    ...
    seller: seller ?? null,
    seller_id: sellerId ?? (sellerObj?.id != null ? String(sellerObj.id) : null),
...
async function catalogOffers(product:ProductDetail,tokens:string[],limit:number):Promise<SearchMlItem[]> {
  ...
  const images=productImages(product),title=String(product.name??product.family_name??"Anúncio Mercado Livre").trim().slice(0,60);
  ...
  return rows.map(row=>({
    id:String(row.item_id),title,price_cents:Math.round(Number(row.price)*100),thumbnail:images[0]??null,permalink:null,category:row.category_id??null,
    seller:row.seller_id!=null?sellers.get(String(row.seller_id))??null:null,seller_id:row.seller_id!=null?String(row.seller_id):null,
    condition:row.condition??null,available_quantity:typeof row.available_quantity==="number"?row.available_quantity:null,sold_quantity:typeof row.sold_quantity==="number"?row.sold_quantity:null,
    status:row.status??"active",images,attributes:Array.isArray(product.attributes)?product.attributes:Array.isArray(product.main_features)?product.main_features:[],source_kind:"catalog_offer" as const,
  }));
}
...
function rankBySales(items:SearchMlItem[]){return[...items].sort((a,b)=>(b.sold_quantity??-1)-(a.sold_quantity??-1));}
...
async function officialCatalogSearch(query:string,tokens:string[],limit:number):Promise<{items:SearchMlItem[];statuses:number[];failed:boolean}> {
  ...
  if(!rows.length){const domains=await discoverDomains(query,tokens);for(const domain of domains){const page=await productSearchPage(query,tokens,0,domain);statuses.push(...page.statuses);failed=failed&&page.failed;rows.push(...page.rows);if(rows.length>=productTarget)break;}}
...
export const searchMercadoLivre=createServerFn({method:"POST"}).middleware([requireSupabaseAuth]).inputValidator((data:unknown)=>z.object({query:z.string().trim().min(1).max(120),limit:z.number().int().min(1).max(200).optional()}).parse(data)).handler(async({data,context}):Promise<SearchResult>=>{const limit=data.limit??20,tokens=await getTokens(context.userId);if(!tokens.length)return{ok:false,configured:true,reason:"Conecte sua conta do Mercado Livre para usar a busca.",items:[]};const [marketplace,catalog]=await Promise.all([legacyMarketplaceSearch(data.query,tokens,Math.min(limit,50)),officialCatalogSearch(data.query,tokens,limit)]);const merged=rankBySales(Array.from(new Map([...marketplace,...catalog.items].map(item=>[item.id,item])).values())).slice(0,limit);if(merged.length)return{ok:true,configured:true,reason:marketplace.length?null:"Resultados obtidos pelos recursos oficiais do Mercado Livre. O ranking usa vendas somente quando a API informa esse dado.",items:merged};return{ok:false,configured:true,reason:userMessage(catalog.statuses,true),items:[]};});

export const searchMercadoLivreProducts=createServerFn({method:"POST"}).middleware([requireSupabaseAuth]).inputValidator((data:unknown)=>z.object({query:z.string().trim().min(1).max(120),limit:z.number().int().min(1).max(200).optional()}).parse(data)).handler(async({data,context}):Promise<SearchResult>=>{const tokens=await getTokens(context.userId);if(!tokens.length)return{ok:false,configured:true,reason:"Conecte sua conta do Mercado Livre para usar a busca.",items:[]};const result=await officialCatalogSearch(data.query,tokens,data.limit??20);return result.items.length?{ok:true,configured:true,reason:"Resultados obtidos pelos recursos oficiais do Mercado Livre. O ranking usa vendas somente quando a API informa esse dado.",items:result.items}:{ok:false,configured:true,reason:userMessage(result.statuses,true),items:[]};});
