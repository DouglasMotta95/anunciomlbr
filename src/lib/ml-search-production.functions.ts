...
async function getTokens(userId: string): Promise<string[]> {
  ...
  try {
    ...
    // O app token ainda pode permitir a consulta.
  }
  try {
    const app = await getAppAccessToken();
    if (app && !tokens.includes(app)) tokens.push(app);
  } catch {
    // Alguns endpoints ainda podem funcionar sem token.
  }
  return tokens;
}
...
async function sellerNickname(
  ...
  const data = (await attempt.response.json().catch(() => null)) as
    | { nickname?: unknown }
    | null;
  const value =
    typeof data?.nickname === "string" && data.nickname.trim() ? data.nickname.trim() : null;
  sellerCache.set(key, { value, expires: Date.now() + 10 * 60_000 });
  return value;
}

async function mapItemRaw(
  raw: Record<string, unknown>,
  tokens: string[],
  sourceKind: SearchMlItem["source_kind"] = "marketplace",
  ...
  const seller =
    typeof sellerObj?.nickname === "string"
      ? sellerObj.nickname
      : await sellerNickname(resolvedSellerId, tokens);

  return {
    id: String(raw["id"] ?? raw["item_id"] ?? ""),
    title: String(raw["title"] ?? "Anúncio Mercado Livre"),
    price_cents:
      typeof raw["price"] === "number" ? Math.round(Number(raw["price"]) * 100) : null,
...
async function fetchItemsBatch(ids: string[], tokens: string[]): Promise<SearchMlItem[]> {
  const unique = Array.from(
    new Set(
      ids
        .map((id) => id.toUpperCase().replace("MLB-", "MLB"))
...
async function fetchItem(
  itemId: string,
  tokens: string[],
): Promise<{ item: SearchMlItem | null; statuses: number[] }> {
  const id = itemId.toUpperCase().replace("MLB-", "MLB");
  const direct = await mlFetch(
    `${ML_API}/items/${encodeURIComponent(id)}?include_attributes=all`,
    tokens,
  );
  if (direct.response?.ok) {
    const raw = (await direct.response.json().catch(() => null)) as
...
async function descriptionResult(
  itemId: string,
  tokens: string[],
): Promise<{ description: string | null; reason: string | null }> {
  ...
  if (attempt.response?.ok) {
    const data = (await attempt.response.json().catch(() => null)) as
      ...
      | null;
    const description =
      typeof data?.plain_text === "string" && data.plain_text.trim()
        ? data.plain_text.trim()
        : null;
    return {
      description,
      reason: description ? null : "Este anúncio não possui descrição disponível.",
    };
  }
  if (attempt.statuses.includes(404)) {
    return { description: null, reason: "Este anúncio não possui descrição disponível." };
  }
...
async function catalogOffers(
  ...
  const data = (await attempt.response.json().catch(() => null)) as
    | { results?: CatalogOfferRow[] }
    | null;
  const rows = (data?.results ?? [])
    .filter((row) => typeof row.item_id === "string" && typeof row.price === "number")
    .slice(0, limit);
  if (!rows.length) return [];

  const verified = await fetchItemsBatch(
    rows.map((row) => String(row.item_id)),
    tokens,
  );
  const verifiedById = new Map(verified.map((item) => [item.id.toUpperCase(), item]));
  const images = productImages(product);
  const title = String(product.name ?? product.family_name ?? "Oferta de catálogo")
    .trim()
    .slice(0, 60);
  const sellerIds = Array.from(
    new Set(
      rows
        .map((row) => (row.seller_id != null ? String(row.seller_id) : null))
        .filter((value): value is string => !!value),
    ),
  );
  const sellerPairs = await Promise.all(
    sellerIds.map(async (id) => [id, await sellerNickname(id, tokens)] as const),
  );
  const sellers = new Map(sellerPairs);
  const fallbackAttributes = serializeMlArray(
    Array.isArray(product.attributes)
      ? product.attributes
      : Array.isArray(product.main_features)
  ...
  return rows.map((row): SearchMlItem => {
    ...
    if (detail) {
      return {
        ...
        sold_quantity:
          ...
          (typeof row.sold_quantity === "number" ? row.sold_quantity : null),
        available_quantity:
    ...
    return {
      ...
      seller:
        row.seller_id != null ? sellers.get(String(row.seller_id)) ?? null : null,
...
async function productSearchPage(
  ...
  const attempt = await mlFetch(url, tokens);
  if (!attempt.response?.ok) {
    return { rows: [] as ProductSearchRow[], statuses: attempt.statuses, failed: true };
  }
  const data = (await attempt.response.json().catch(() => null)) as
...
function rankBySales(items: SearchMlItem[]) {
  return [...items].sort(
    (a, b) =>
      Number(b.verified_item !== false) - Number(a.verified_item !== false) ||
      (b.sold_quantity ?? -1) - (a.sold_quantity ?? -1),
  );
}

async function officialCatalogSearch(
  query: string,
  tokens: string[],
  limit: number,
): Promise<{ items: SearchMlItem[]; statuses: number[]; failed: boolean }> {
  ...
  return {
    items: rankBySales(unique).slice(0, desired),
    statuses,
    failed: failed && !unique.length,
  };
}

async function legacyMarketplaceSearch(
  query: string,
  tokens: string[],
  limit: number,
): Promise<SearchMlItem[]> {
  ...
  const data = (await attempt.response.json().catch(() => null)) as
    ...
    | null;
  const mapped = await Promise.all(
    (data?.results ?? []).slice(0, desired).map((row) => mapItemRaw(row, tokens, "marketplace")),
  );
...
async function sellerSearch(
  ...
  if (/^\d+$/.test(value)) {
    ...
    for (let offset = 0; offset < desired; offset += 50) {
      ...
      if (page.length) {
        const mapped = await Promise.all(
          page.map((row) => mapItemRaw(row, tokens, "marketplace")),
        );
...
function userMessage(statuses: number[], empty = false) {
  ...
  if (statuses.includes(403)) {
    return empty
      ? "A API do Mercado Livre não liberou ofertas verificáveis para este termo. Tente outro termo, link, ID ou vendedor."
      : "Não foi possível consultar esse tipo de anúncio agora.";
  }
  return empty
    ? "Não encontramos ofertas verificáveis para este termo."
...
async function resolveLink(raw: string): Promise<{ itemId: string | null; productId: string | null }> {
  ...
  if (
    /^(?:www\.)?(?:produto\.|lista\.)?mercadolivre\.com\.br\//i.test(candidate) ||
    /^(?:www\.)?meli\.la\//i.test(candidate)
  ) {
  ...
  for (let hop = 0; hop < 5; hop += 1) {
    ...
    if (id) {
      return /\/p\/MLB-?\d+/i.test(current.pathname)
        ? { itemId: null, productId: id }
        : { itemId: id, productId: null };
    }
    try {
  ...
  .handler(async ({ data, context }): Promise<SearchResult> => {
    ...
    if (!tokens.length) {
      return {
        ok: false,
        configured: true,
        reason: "Conecte sua conta do Mercado Livre para usar a busca.",
        items: [],
      };
    }
    ...
    const merged = rankBySales(
      Array.from(
        new Map([...marketplace, ...catalog.items].map((item) => [item.id, item])).values(),
      ),
    ).slice(0, limit);
    if (merged.length) {
      const verified = merged.filter((item) => item.verified_item !== false).length;
      return {
        ok: true,
        configured: true,
        reason: marketplace.length
          ? null
          : verified
            ? "Resultados obtidos pelos recursos oficiais do Mercado Livre. Ofertas verificadas aparecem primeiro; o ranking usa vendas quando a API informa esse dado."
            : "A API retornou referências de catálogo, mas não liberou os itens completos. Essas referências ficam sem preço copiável até serem verificadas.",
        items: merged,
      };
    }
  ...
  .handler(async ({ data, context }): Promise<SearchResult> => {
    ...
    return result.items.length
      ? {
          ok: true,
          configured: true,
          reason: "Resultados obtidos pelos recursos oficiais do Mercado Livre. Ofertas verificadas aparecem primeiro.",
          items: result.items,
        }
      : {
          ok: false,
          configured: true,
          reason: userMessage(result.statuses, true),
          items: [],
        };
  });
...
export const getMercadoLivreItem = createServerFn({ method: "POST" })
  ...
  .inputValidator((data: unknown) =>
    z
      .object({ id: z.string().trim().regex(/^MLB-?\d+$/i, "ID inválido. Use MLB1234567890.") })
      .parse(data),
  )
  .handler(async ({ data, context }): Promise<SearchResult> => {
    const tokens = await getTokens(context.userId);
    if (!tokens.length) {
      return {
        ok: false,
        configured: true,
        reason: "Conecte sua conta do Mercado Livre para consultar o anúncio.",
        items: [],
      };
    }
    const result = await fetchItem(data.id, tokens);
    return result.item
      ? { ok: true, configured: true, reason: null, items: [result.item] }
      : {
          ok: false,
          configured: true,
          reason: userMessage(result.statuses, false),
          items: [],
        };
  });
...
export const getMercadoLivreItemDescription = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: unknown) =>
    z.object({ id: z.string().trim().regex(/^MLB-?\d+$/i) }).parse(data),
  )
  .handler(async ({ data, context }) => {
    ...
    if (!tokens.length) {
      return {
        ok: false as const,
        description: null,
        reason: "Conecte sua conta do Mercado Livre.",
      };
    }
...
export const getMercadoLivreItemFromLink = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((data: unknown) =>
    z.object({ link: z.string().trim().min(4).max(1000) }).parse(data),
  )
  .handler(async ({ data, context }): Promise<SearchResult> => {
    const tokens = await getTokens(context.userId);
    if (!tokens.length) {
      return {
        ok: false,
        configured: true,
        reason: "Conecte sua conta do Mercado Livre para consultar o link.",
        items: [],
      };
    }
    ...
    if (resolved.productId) {
      const product = await productDetail(resolved.productId, tokens);
      const offers = product ? await catalogOffers(product, tokens, 1) : [];
      return offers[0]
        ? {
            ok: true,
            configured: true,
            reason:
              offers[0].verified_item === false
                ? "O produto foi identificado, mas a API não liberou o anúncio completo para copiar."
                : null,
            items: [offers[0]],
          }
        : {
            ok: false,
            configured: true,
            reason: "O produto foi identificado, mas não retornou uma oferta ativa.",
            items: [],
          };
    }
    return {
      ok: false,
      configured: true,
      reason: "Não conseguimos identificar um anúncio nesse link.",
      items: [],
    };
  });
