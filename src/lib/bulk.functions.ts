...
const startSchema = z.object({
  kind: z.enum(["copy", "duplicate", "optimize", "pause", "activate", "delete"]),
...
export const startBulkJob = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: job, error } = await supabaseAdmin
      .from("bulk_jobs")
      .insert({ user_id: context.userId, kind: data.kind, status: "queued", total: items.length, processed: 0, failed: 0, payload: { items } as never })
      .select("id")
      .single();
...
async function checkBulkAiCredit(userId: string) {
  ...
  if (quota.remaining < 1) {
    throw new Error(`Créditos de IA esgotados (${quota.used}/${quota.credit_limit}). Compre créditos extras de IA na Central da assinatura ou faça upgrade.`);
  }
}
...
async function runBulkItem(kind: BulkJobKind, userId: string, item: BulkJobItem) {
  ...
  if (kind === "copy") {
    ...
    const { data: existing } = await supabaseAdmin.from("listings").select("id").eq("user_id", userId).eq("source_ml_id", item.id).maybeSingle();
    if (existing?.id) return;
    const { data: created, error } = await supabaseAdmin
      .from("listings")
      .insert({
        user_id: userId,
        title: String(source["title"] ?? item.label).replace(/\s*\((?:copy|cópia)\)\s*$/i, "").slice(0, 60),
  ...
  if (kind === "duplicate") {
    const { data: listing, error: fetchError } = await supabaseAdmin
      .from("listings")
      .select("title,description,price_cents,stock,sku,category,condition,images,attributes,cost_cents,fees_cents,ai_score,source_permalink")
      .eq("id", item.id)
      .eq("user_id", userId)
      .maybeSingle();
    ...
    const { data: created, error } = await supabaseAdmin
      .from("listings")
      .insert({ ...listing, user_id: userId, status: "draft", title: cleanOptimizedTitle(String(listing.title ?? item.label)) })
      .select("id")
      .single();
  ...
  if (kind === "optimize") {
    ...
    const out = await aiJson<{ title?: string; description?: string; score_after?: number }>(
      optimizationPrompt({
        title: listing.title,
    ...
    const { error: updateError } = await supabaseAdmin
      ...
      .update({
        ...
        ai_score: Number.isFinite(parsed.score_after) ? Math.max(0, Math.min(100, Number(parsed.score_after))) : null,
