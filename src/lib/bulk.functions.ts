...
export type BulkJobKind = "copy" | "duplicate" | "optimize" | "pause" | "activate" | "archive" | "delete";
const startSchema = z.object({
  kind: z.enum(["copy", "duplicate", "optimize", "pause", "activate", "archive", "delete"]),
  items: z.array(z.object({ id: z.string().min(1), label: z.string().min(1), source: z.record(z.string(), z.unknown()).optional().nullable() })).min(1).max(200),
});
...
export const getBulkJob = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { data: job, error } = await context.supabase.from("bulk_jobs").select("*").eq("id", data.jobId).eq("user_id", context.userId).maybeSingle();
...
async function processBulkJob(jobId: string, userId: string, kind: BulkJobKind, items: BulkJobItem[]) {
  ...
  for (const item of state) {
    item.status = "processing"; await persist();
    try { await runBulkItem(kind, userId, item); item.status = "done"; item.message = null; processed += 1; }
    catch (error) { item.status = "error"; item.message = error instanceof Error ? error.message : "Erro inesperado"; failed += 1; }
    await persist();
  }
...
function sourceImages(source: Record<string, unknown>): string[] { const provided = Array.isArray(source["images"]) ? (source["images"] as unknown[]).map(normalizeHttps).filter((v): v is string => !!v) : []; if (provided.length) return Array.from(new Set(provided)); const thumbnail = normalizeHttps(source["thumbnail"]); return thumbnail ? [thumbnail] : []; }
...
async function claimListingQuota(userId: string, listingId: string) {
  ...
  if (error || data !== true) {
    ...
    throw new Error(error ? "Não foi possível validar sua franquia de criações agora. Tente novamente." : "Franquia de criações e clonagens deste ciclo atingida. Seus anúncios existentes continuam ativos; compre anúncios extras na Central da assinatura ou faça upgrade.");
  }
}

async function checkBulkAiCredit(userId: string) { const { getAiQuota } = await import("./ai-quota.server"); const quota = await getAiQuota(userId); if (quota.remaining < 1) throw new Error(`Créditos de IA esgotados (${quota.used}/${quota.credit_limit}). Compre créditos extras de IA na Central da assinatura ou faça upgrade.`); }
async function consumeBulkAiCredit(userId: string) { const { consumeAiQuota } = await import("./ai-quota.server"); const result = await consumeAiQuota(userId, 1); if (!result.ok) throw new Error(result.reason); }
...
async function runBulkItem(kind: BulkJobKind, userId: string, item: BulkJobItem) {
  ...
  if (kind === "copy") {
    ...
    const { data: created, error } = await supabaseAdmin.from("listings").insert({ user_id: userId, title: String(source["title"] ?? item.label).replace(/\s*\((?:copy|cópia)\)\s*$/i, "").slice(0, 60), price_cents: priceCents, category: source["category"] as string ?? null, condition: source["condition"] as string ?? null, status: "draft", source_ml_id: item.id, source_permalink: source["permalink"] as string ?? null, images: sourceImages(source) as unknown as never, attributes: attributes as unknown as never, stock: typeof source["available_quantity"] === "number" ? source["available_quantity"] as number : 1 }).select("id").single();
    if (error || !created) throw new Error(error?.message ?? "Não foi possível clonar o anúncio."); await claimListingQuota(userId, created.id); return;
  }
  if (kind === "duplicate") {
    const { data: listing, error: fetchError } = await supabaseAdmin.from("listings").select("title,description,price_cents,stock,sku,category,condition,images,attributes,cost_cents,fees_cents,ai_score,source_permalink").eq("id", item.id).eq("user_id", userId).maybeSingle();
    if (fetchError || !listing) throw new Error("Anúncio não encontrado.");
    const { cleanOptimizedTitle } = await import("./ai.server");
    const { data: created, error } = await supabaseAdmin.from("listings").insert({ ...listing, user_id: userId, status: "draft", title: cleanOptimizedTitle(String(listing.title ?? item.label)) }).select("id").single();
    if (error || !created) throw new Error(error?.message ?? "Não foi possível criar a cópia."); await claimListingQuota(userId, created.id); return;
  }
  ...
  if (kind === "pause" || kind === "activate" || kind === "archive") {
    ...
    const { error } = await supabaseAdmin.from("listings").update({ status: nextStatus, updated_at: new Date().toISOString() }).eq("id", item.id).eq("user_id", userId);
    if (error) throw new Error(error.message); return;
  }
  if (kind === "optimize") {
    const { data: listing, error: fetchError } = await supabaseAdmin.from("listings").select("id,title,description,category,price_cents,attributes,images").eq("id", item.id).eq("user_id", userId).maybeSingle();
    if (fetchError || !listing) throw new Error("Anúncio não encontrado.");
    ...
    const out = await aiJson<{ title?: string; description?: string; score_after?: number }>(optimizationPrompt({ title: listing.title, description: listing.description, category: listing.category, price_cents: listing.price_cents, attributes: listing.attributes, images_count: imagesCount }));
    if (!out.ok) throw new Error(out.reason); const parsed = out.result; if (!parsed.title || !parsed.description) throw new Error("A IA não retornou título e descrição válidos.");
    ...
    const { error: updateError } = await supabaseAdmin.from("listings").update({ title: cleanTitle, description: parsed.description.trim(), ai_score: Number.isFinite(parsed.score_after) ? Math.max(0, Math.min(100, Number(parsed.score_after))) : null, updated_at: new Date().toISOString() }).eq("id", item.id).eq("user_id", userId);
