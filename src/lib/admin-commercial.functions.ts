...
export const generateRecoveryMessage=createServerFn({method:'POST'}).middleware([requireSupabaseAuth]).validator((d:unknown)=>z.object({payment_id:z.string().uuid(),email:z.string().email().nullish(),plan:z.string().max(100).nullish(),amount_cents:z.number().int().min(0),status:z.enum(['pending','rejected','cancelled'])}).parse(d)).handler(async({data,context})=>{
 ...
 const {error}=await supabaseAdmin.from('sales_recovery_actions').insert({payment_id:data.payment_id,admin_user_id:context.userId,action:'generated',message:out.result.message})
 if(error)throw new Error('A mensagem foi gerada, mas não foi possível registrar a ação comercial.')
 return {ok:true as const,...out.result}
})

export const markRecoveryAction=createServerFn({method:'POST'}).middleware([requireSupabaseAuth]).validator((d:unknown)=>z.object({payment_id:z.string().uuid(),action:z.enum(['contacted','resolved','ignored']),note:z.string().max(500).nullish()}).parse(d)).handler(async({data,context})=>{
