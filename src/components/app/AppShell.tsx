    ...
    <nav className="flex flex-col gap-5">
      {NAV_GROUPS.map((group) => (
        <div key={group.label} className="space-y-1.5">
          <p className="px-3 text-[10px] font-bold uppercase tracking-[0.18em] text-muted-foreground/60">
            {group.label}
          </p>
          {group.items.map((item) => {
            ...
            const className = cn(
              "group flex items-center gap-3 rounded-2xl px-3 py-2.5 text-sm font-medium text-muted-foreground transition-all duration-200 hover:bg-muted/70 hover:text-foreground",
            ...
            const content = (
              <>
                <span
                  className={cn(
                    "flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-muted/80 transition-colors group-hover:bg-background",
                    active && "bg-primary-foreground/15 group-hover:bg-primary-foreground/15",
                  )}
                >
      ...
      {license?.plan ? (
        ...
        <>
          ...
          <p className="mt-3 text-sm font-semibold text-foreground">
            {(profile?.free_listings_limit ?? 10) - (profile?.free_listings_used ?? 0)} anúncios disponíveis
          </p>
      ...
      <div className="mx-auto flex min-h-screen max-w-[1560px]">
        <aside className="sticky top-0 hidden h-screen w-[286px] shrink-0 flex-col border-r border-border/70 bg-background/95 px-4 py-5 backdrop-blur-xl lg:flex">
          ...
          <div className="mt-5 space-y-3 border-t border-border/60 pt-4">
            ...
            <Button variant="ghost" size="sm" className="w-full justify-start rounded-xl text-muted-foreground hover:text-foreground" onClick={signOut}>
        ...
        <main className="min-w-0 flex-1">
            ...
            <div className="flex min-h-[82px] flex-wrap items-center justify-between gap-3 px-4 py-4 sm:px-6 lg:px-8">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="mb-0.5 text-[10px] font-bold uppercase tracking-[0.18em] text-primary/80">ANÚNCIO ML</p>
          ...
          <div className="mx-auto max-w-[1240px] p-4 pb-28 sm:p-6 lg:p-8 lg:pb-8">
            <div className="rounded-3xl border border-border/60 bg-background/80 p-3 shadow-sm sm:p-5 lg:p-6">
              {children}
            </div>
          </div>
        </main>
        ...
        {MOBILE_NAV.map((item) => {
          ...
          return (
            <Link
              ...
              to={item.to}
              className={cn(
                "flex flex-col items-center gap-1 rounded-xl px-1 py-2.5 text-[10px] font-semibold text-muted-foreground transition-colors",
