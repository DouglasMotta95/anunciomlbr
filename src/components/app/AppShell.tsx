      ...
      {license?.plan ? (
        <>
          ...
          <p className="mt-3 text-sm font-semibold">
            {isLoading ? "Carregando uso…" : `${remaining} de ${total} anúncios disponíveis`}
          </p>
          ...
          {days !== null && days >= 0 && (
            <p className="mt-2 text-xs text-muted-foreground">{days} dia(s) restantes no período atual</p>
          )}
          <Button asChild size="sm" variant="outline" className="mt-3 w-full rounded-xl bg-background/50">
            <Link to="/assinatura">
        ...
        <>
          ...
          <p className="mt-3 text-sm font-semibold">
            {isLoading ? "Carregando uso…" : `${remaining} de ${total || 10} anúncios disponíveis`}
          </p>
          <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
            Criações e cópias usam a mesma franquia.
          </p>
          <Button asChild size="sm" className="mt-3 w-full rounded-xl shadow-sm">
            <Link to="/assinatura">
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,oklch(0.6_0.19_258/.08),transparent_28%),radial-gradient(circle_at_top_right,oklch(0.86_0.17_92/.06),transparent_24%)]">
      <div className="mx-auto flex min-h-screen max-w-[1600px]">
        <aside className="sticky top-0 hidden h-screen w-[292px] shrink-0 flex-col border-r border-border/60 bg-background/88 px-4 py-5 shadow-[18px_0_60px_-52px_oklch(0.05_0.03_265)] backdrop-blur-2xl lg:flex">
          <div className="rounded-2xl border border-border/55 bg-gradient-to-br from-card/95 to-card/65 px-3 py-3 shadow-sm">
            <Logo />
          </div>
          ...
          <div className="mt-5 space-y-3 border-t border-border/60 pt-4">
            <PlanCard />
            <div className="rounded-2xl border border-border/50 bg-card/45 px-3 py-2.5">
              <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-muted-foreground">Conta</p>
              <p className="mt-1 truncate text-xs font-semibold text-foreground/90">{user?.email}</p>
            </div>
            <Button
              variant="ghost"
              size="sm"
        ...
        <main className="min-w-0 flex-1">
              ...
              <div className="flex min-w-0 items-center gap-3">
                <Sheet open={open} onOpenChange={setOpen}>
                  <SheetTrigger asChild>
                    <Button variant="outline" size="icon" className="shrink-0 rounded-xl bg-card/60 lg:hidden">
                ...
                <div className="min-w-0">
                  <div className="mb-1 flex items-center gap-2">
                    <span className="h-1.5 w-1.5 rounded-full bg-primary shadow-[0_0_14px_oklch(0.86_0.17_92/.8)]" />
                    <p className="text-[10px] font-extrabold uppercase tracking-[0.2em] text-primary/85">ANÚNCIO ML</p>
                  </div>
                  <h1 className="truncate font-display text-xl font-extrabold tracking-tight sm:text-2xl">{title}</h1>
                  {description && (
                    <p className="mt-1 max-w-2xl text-xs leading-relaxed text-muted-foreground sm:text-sm">{description}</p>
                  )}
                </div>
              </div>
          ...
          <div className="mx-auto max-w-[1280px] p-3 pb-28 sm:p-6 lg:p-8 lg:pb-10">
            <div className="overflow-hidden rounded-[1.75rem] border border-border/55 bg-background/72 p-3 shadow-[0_28px_80px_-56px_oklch(0.05_0.03_265)] backdrop-blur-sm sm:p-5 lg:p-6">
              {children}
            </div>
          </div>
        </main>
        ...
        {MOBILE_NAV.map((item) => {
          ...
          return (
            <Link
              ...
              to={item.to}
              className={cn(
                "flex min-h-14 select-none touch-manipulation flex-col items-center justify-center gap-1 rounded-xl px-1 py-2 text-[10px] font-semibold text-muted-foreground transition-all duration-200 active:scale-95",
