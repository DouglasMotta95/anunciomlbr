...
import {
  BadgeDollarSign, BarChart3, CreditCard, FileText, HeartPulse, KeyRound, LayoutDashboard,
  LifeBuoy, LogOut, Package, Plug, ScrollText, Settings, Store, Ticket, UserMinus, Users, Radar,
} from "lucide-react";
...
export function AdminLayout({ activeSection, onSectionChange, children }: { activeSection: string; onSectionChange: (section: string) => void; children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-background"><div className="mx-auto flex max-w-[1480px]">
    <aside className="sticky top-0 hidden h-screen w-[286px] shrink-0 flex-col justify-between overflow-y-auto border-r border-border bg-card/30 p-5 lg:flex">
      <div className="space-y-6"><Logo to="/admin"/><div className="rounded-lg border border-primary/20 bg-primary/10 px-3 py-2 text-xs font-semibold uppercase tracking-wider text-primary">Central administrativa</div>
      <nav className="space-y-5">{ADMIN_NAV_GROUPS.map((group)=><div key={group.title} className="space-y-1"><p className="px-3 pb-1 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground/70">{group.title}</p>{group.items.map((item)=>{const Icon=item.icon;const active=activeSection===item.section;return <button key={item.section} type="button" onClick={()=>onSectionChange(item.section)} className={cn("flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground",active&&"bg-primary/10 text-foreground ring-1 ring-primary/25")}><Icon className={cn("h-4 w-4",active&&"text-primary")}/>{item.label}</button>})}</div>)}</nav></div>
    ...
    <main className="min-w-0 flex-1"><header className="sticky top-0 z-20 border-b border-border bg-background/90 px-5 py-4 backdrop-blur-xl"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wider text-primary">ANÚNCIO ML</p><h1 className="font-display text-xl font-bold tracking-tight">{current?.label??"Painel administrativo"}</h1><p className="text-sm text-muted-foreground">Controle de clientes, receita, licenças, integrações e saúde operacional.</p></div><div className="flex items-center gap-2 lg:hidden"><Button asChild variant="outline" size="sm"><Link to={pathname==="/admin"?"/dashboard":"/admin"}>{pathname==="/admin"?"Cliente":"Admin"}</Link></Button><Button variant="ghost" size="icon" onClick={signOut} aria-label="Sair"><LogOut className="h-4 w-4"/></Button></div></div><div className="mt-3 lg:hidden"><Select value={activeSection} onValueChange={onSectionChange}><SelectTrigger aria-label="Seção do painel"><SelectValue placeholder="Selecione a seção"/></SelectTrigger><SelectContent>{ADMIN_NAV_GROUPS.flatMap((group)=>group.items.map((item)=><SelectItem key={item.section} value={item.section}>{group.title} · {item.label}</SelectItem>))}</SelectContent></Select></div></header><div className="p-4 pb-10 sm:p-5">{children}</div></main>
  </div></div>;
}
