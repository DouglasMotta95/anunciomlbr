    ...
    >
      <div className="grid gap-4 xl:grid-cols-[1.2fr_.8fr]">
        <Card className="border-primary/20">
          <CardHeader>
            <CardTitle className="text-base">Seu acesso atual</CardTitle>
          </CardHeader>
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {license?.expires_at
                      ...
                      : "Sem limite por tempo; teste limitado por quantidade"}
                  </p>
                </div>
              </div>
              {license?.plan ? (
                ...
                <Badge variant="secondary">10 anúncios para testar</Badge>
              )}
            </div>

            <Usage
              label="Criações e duplicações"
              used={adsUsed}
              total={adsTotal}
              remaining={adsRemaining}
              detail="Cada novo anúncio criado ou duplicado consome 1 unidade. Editar ou publicar o mesmo rascunho não consome novamente."
            />
            <Usage
              label="Créditos de IA"
              used={Number(ai.used ?? 0)}
              total={Number(ai.limit ?? 0)}
              remaining={Number(ai.remaining ?? 0)}
              detail="Otimizações, sugestões de resposta e demais ações de IA usam um saldo separado."
            />

            <div className="grid gap-2 sm:grid-cols-2">
              <Button asChild variant="outline">
                <Link to="/creditos">
                  <PackagePlus className="mr-2 h-4 w-4" />Comprar anúncios extras
                </Link>
              </Button>
              <Button asChild variant="outline">
                <Link to="/creditos-ia">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Ativar uma chave</CardTitle>
          </CardHeader>
          <CardContent>
            <form
              className="space-y-3"
            ...
            >
              <div className="space-y-2">
                <Label htmlFor="code">Código da licença</Label>
                <Input
                  id="code"
                  value={code}
                  placeholder="AML-XXXX-XXXX-XXXX"
            ...
            {license?.code && (
              <div className="mt-4 rounded-xl border p-3">
                <p className="text-xs text-muted-foreground">Licença atual</p>
                <div className="mt-1 flex items-center justify-between gap-2">
                  <code className="truncate text-xs">{license.code}</code>
                  <Button
                    size="icon"
                    variant="ghost"
                    onClick={() => {
            ...
            return (
              ...
              >
                  ...
                  <div className="pt-2">
                    ...
                    {discount && discount.months > 1 && (
                      <p className="mt-1 text-xs text-muted-foreground">
                        {formatBRL(total)} no período
                        {savings > 0 ? ` · economize ${formatBRL(savings)}` : ""}
                      </p>
                    )}
                  </div>
                </CardHeader>
                <CardContent className="flex flex-1 flex-col gap-4">
                  ...
                  {!isCurrent && (
                    <div className="rounded-xl border border-primary/15 bg-primary/[.04] p-3 text-[11px] leading-5 text-muted-foreground">
                      Mercado Pago seguro · saldo, Pix, cartão ou outro meio disponível no checkout.
                    </div>
                  )}
                  <Button
                    className="w-full"
                  ...
                  >
                    ...
                    {isCurrent ? "Plano atual" : "Continuar para pagamento seguro"}
                  </Button>
                </CardContent>
              </Card>
            );
  ...
  return (
    <div className="rounded-2xl border bg-muted/20 p-4">
      ...
      <div className="mt-2 flex items-start justify-between gap-3">
        <p className="max-w-xl text-xs leading-5 text-muted-foreground">{detail}</p>
