...
import { CheckCircle2, Loader2, PackagePlus, ShoppingCart, TrendingUp } from "lucide-react";
...
function CreditsPage() {
  ...
  const { data, isLoading, refetch } = useQuery({ queryKey: ["extra-ad-packages"], queryFn: () => load() });
  ...
  useEffect(() => {
    ...
    void confirm({ data: { payment_id: search.payment_id } }).then(async (result) => {
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
        <div className="space-y-4">
          ...
          {!data?.eligible && !isLoading ? (
            <Card className="border-warning/40"><CardContent className="space-y-3 p-5"><p className="font-semibold">Pacotes extras são exclusivos para clientes com plano ativo.</p><p className="text-sm text-muted-foreground">Se os 10 anúncios gratuitos acabaram, primeiro escolha um plano. Depois você poderá comprar créditos avulsos sempre que precisar.</p><Button asChild><Link to="/licenca">Escolher um plano</Link></Button></CardContent></Card>
          ) : (
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              {(data?.packages ?? []).map((pack) => (
                <Card key={pack.id} className={pack.highlighted ? "border-primary/50 ring-1 ring-primary/20" : ""}>
                  <CardHeader className="space-y-2"><div className="flex items-center justify-between"><PackagePlus className="h-5 w-5 text-primary" />{pack.badge && <Badge variant="secondary">{pack.badge}</Badge>}</div><CardTitle className="text-lg">{pack.name}</CardTitle><p className="text-xs text-muted-foreground">{pack.tagline}</p></CardHeader>
                  <CardContent className="space-y-4"><div><p className="text-2xl font-extrabold">{formatBRL(pack.price_monthly_cents)}</p><p className="text-xs text-muted-foreground">pagamento único</p></div><div className="flex items-center gap-2 text-sm"><CheckCircle2 className="h-4 w-4 text-success" /><span>{pack.ad_quota} anúncios adicionados ao saldo</span></div><Button className="w-full" variant={pack.highlighted ? "default" : "outline"} disabled={purchase.isPending} onClick={() => purchase.mutate(pack.id)}>{purchase.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <ShoppingCart className="mr-2 h-4 w-4" />}Comprar</Button></CardContent>
                </Card>
              ))}
            </div>
          )}
        ...
        <Card className="h-fit"><CardHeader><CardTitle className="text-base">Como funciona</CardTitle></CardHeader><CardContent className="space-y-3 text-sm text-muted-foreground"><p>1. Seu plano continua igual.</p><p>2. Você compra apenas a quantidade extra que precisa.</p><p>3. O pagamento é feito pelo Mercado Pago.</p><p>4. Após a aprovação, você volta para esta tela e o saldo é recarregado automaticamente.</p><p>5. Os créditos extras são consumidos junto com a sua cota de anúncios.</p><Button asChild variant="outline" className="w-full"><Link to="/licenca">Ver upgrade de plano</Link></Button></CardContent></Card>
      </div>
    </AppShell>
  );
}
