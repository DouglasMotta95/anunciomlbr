...
import { Activity, AlertTriangle, Bot, Calculator, Check, Copy, ExternalLink, Eye, Gift, Loader2, Radar, RefreshCw, Sparkles, Target, Trash2, TrendingDown, TrendingUp } from "lucide-react";
...
import { addCompetitorWatch, calculateSmartPrice, getReferralSummary, getSellerGrowthOverview, listCompetitorWatch, refreshCompetitorWatch, removeCompetitorWatch } from "@/lib/seller-growth.functions";
...
function GrowthPage() {
  const overviewFn = useServerFn(getSellerGrowthOverview), referralFn = useServerFn(getReferralSummary), radarFn = useServerFn(listCompetitorWatch), addRadarFn = useServerFn(addCompetitorWatch), refreshRadarFn = useServerFn(refreshCompetitorWatch), removeRadarFn = useServerFn(removeCompetitorWatch), calcFn = useServerFn(calculateSmartPrice), copilotFn = useServerFn(askSellerCopilot);
  ...
  const addRadar = useMutation({ mutationFn: () => addRadarFn({ data: { ml_item_id: mlb.trim().toUpperCase() } }), onSuccess: async () => { setMlb(""); await refreshRadarFn({ data: {} }).catch(()=>null); void qc.invalidateQueries({ queryKey: ["competitor-watch"] }); toast.success("Anúncio adicionado ao radar"); }, onError: (error) => toast.error(error instanceof Error ? error.message : "Use um ID válido, como MLB123456789") });
  const refreshRadar = useMutation({ mutationFn: (id?: string) => refreshRadarFn({ data: id ? { id } : {} }), onSuccess: (result) => { void qc.invalidateQueries({ queryKey: ["competitor-watch"] }); toast.success(`${result.updated} concorrente(s) atualizado(s)`, { description: result.changes ? `${result.changes} mudança(s) detectada(s).` : "Nenhuma mudança detectada nesta consulta." }); }, onError: (error) => toast.error(error instanceof Error ? error.message : "Não foi possível atualizar o radar agora.") });
  ...
  return <AppShell title="Central de crescimento" description="Prioridades objetivas, margem, concorrência e IA — sem misturar estimativas com resultados reais.">
    ...
    <Card className="mt-4"><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle className="flex items-center gap-2 text-base"><Radar className="h-4 w-4 text-primary"/>Radar de concorrentes</CardTitle><p className="mt-1 text-xs text-muted-foreground">Acompanhe preço, estoque, vendas acumuladas e mudanças dos anúncios observados.</p></div><Button size="sm" variant="outline" disabled={refreshRadar.isPending || !radar.length} onClick={() => refreshRadar.mutate(undefined)}>{refreshRadar.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <RefreshCw className="mr-2 h-4 w-4"/>}Atualizar radar</Button></div></CardHeader><CardContent className="space-y-4"><div className="flex flex-col gap-2 sm:flex-row"><Input value={mlb} onChange={(e) => setMlb(e.target.value)} placeholder="MLB123456789"/><Button disabled={addRadar.isPending || mlb.trim().length < 5} onClick={() => addRadar.mutate()}>{addRadar.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <Eye className="mr-2 h-4 w-4"/>}Acompanhar</Button></div>{radarLoading ? <p className="text-sm text-muted-foreground">Carregando…</p> : radar.length === 0 ? <div className="rounded-xl border border-dashed p-5 text-sm text-muted-foreground">Nenhum concorrente acompanhado ainda.</div> : <div className="grid gap-3 lg:grid-cols-2">{radar.map((row: any) => {
      const delta=typeof row.price_delta_cents==="number"?row.price_delta_cents:null,DeltaIcon=delta!=null&&delta<0?TrendingDown:TrendingUp;
      return <div key={row.id} className="rounded-xl border p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-mono text-xs text-muted-foreground">{row.ml_item_id}</p>{row.last_status&&<Badge variant={row.last_status==="active"?"outline":"secondary"}>{row.last_status==="active"?"Ativo":row.last_status}</Badge>}{row.status_changed&&<Badge variant="destructive">Status mudou</Badge>}</div>{row.title&&<p className="mt-2 line-clamp-2 text-sm font-semibold">{row.title}</p>}</div><div className="flex shrink-0 gap-1">{row.permalink&&<Button size="icon" variant="ghost" asChild><a href={row.permalink} target="_blank" rel="noopener noreferrer" aria-label="Abrir concorrente"><ExternalLink className="h-4 w-4"/></a></Button>}<Button size="icon" variant="ghost" onClick={() => refreshRadar.mutate(row.id)} disabled={refreshRadar.isPending} aria-label="Atualizar concorrente"><RefreshCw className="h-4 w-4"/></Button><Button size="icon" variant="ghost" onClick={() => removeRadar.mutate(row.id)} disabled={removeRadar.isPending} aria-label="Remover concorrente"><Trash2 className="h-4 w-4"/></Button></div></div><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><Mini label="Preço" value={row.last_price_cents!=null?formatBRL(row.last_price_cents):"—"}/><Mini label="Estoque" value={row.last_available_quantity!=null?formatNumber(row.last_available_quantity):"—"}/><Mini label="Vendidos" value={row.last_sold_quantity!=null?formatNumber(row.last_sold_quantity):"—"}/><Mini label="Variação" value={delta==null?"—":`${delta>0?"+":""}${formatBRL(delta)}`} icon={delta==null?undefined:DeltaIcon}/></div><p className="mt-3 text-[11px] text-muted-foreground">{row.last_checked_at?`Última consulta: ${new Date(row.last_checked_at).toLocaleString("pt-BR")}`:"Ainda não consultado. Atualize o radar para criar o primeiro histórico."}</p></div>;
...
function Mini({label,value,icon:Icon}:{label:string;value:string;icon?:any}){return <div className="rounded-lg bg-muted/30 p-2.5"><p className="text-[10px] uppercase tracking-wide text-muted-foreground">{label}</p><p className="mt-1 flex items-center gap-1 text-xs font-semibold">{Icon&&<Icon className="h-3 w-3"/>}{value}</p></div>;}
