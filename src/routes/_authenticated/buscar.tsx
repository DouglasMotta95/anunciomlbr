...
import { ArrowRight, Clipboard, Download, Edit, ExternalLink, Eye, Files, Loader2, Search, SearchX, ShoppingCart, Sparkles, Store } from "lucide-react";
...
function SearchPage() {
  ...
  return (
    <AppShell title="Buscar e copiar" description="Pesquise anúncios do marketplace, confira dados confirmados e crie uma cópia editável.">
      <section className="overflow-hidden rounded-3xl border border-yellow-400/30 bg-gradient-to-br from-yellow-400/15 via-card to-card shadow-sm">
        <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1.35fr_.65fr] lg:items-center">
          <div>
            ...
            <h2 className="mt-3 max-w-2xl text-2xl font-extrabold tracking-tight sm:text-3xl">Pesquise como no marketplace e copie o anúncio que interessa.</h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">A busca comum consulta itens do site MLB. Catálogo de produtos não é usado como substituto silencioso. Você também pode buscar por código MLB, link ou vendedor.</p>
            ...
            <p className="mt-2 text-xs text-muted-foreground">Vendas só são exibidas quando o Mercado Livre confirma o número pela API. A posição do card não é tratada como ranking do marketplace.</p>
          </div>
      ...
      {!!items.length && <>
        <div className="mt-6 flex flex-col gap-3 rounded-2xl border bg-card px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><Checkbox checked={selectedIds.length === items.length} onCheckedChange={(checked) => setSelected(checked ? Object.fromEntries(items.map((item) => [item.id, true])) : {})} id="select-all"/><label htmlFor="select-all" className="cursor-pointer text-sm font-semibold">{items.length} anúncio(s) encontrado(s)</label></div><div className="flex items-center gap-2"><p className="text-xs text-muted-foreground">Busca de itens do marketplace.</p>{marketplaceSearchUrl&&<Button size="sm" variant="outline" asChild><a href={marketplaceSearchUrl} target="_blank" rel="noopener noreferrer"><ExternalLink className="mr-1.5 h-3.5 w-3.5"/>Comparar no ML</a></Button>}</div></div>
        <div className="mt-3 grid gap-4 pb-28 sm:grid-cols-2 xl:grid-cols-3">{items.map((item) => { const image = getProductImage(item); const hasSales = item.verified_item === true && item.sold_quantity != null; return <Card key={item.id} className="group overflow-hidden rounded-3xl border-border/70 transition-all duration-300 hover:-translate-y-1 hover:border-yellow-400/60 hover:shadow-lg"><CardContent className="p-0"><div className="relative aspect-[4/3] overflow-hidden border-b bg-white"><Checkbox checked={!!selected[item.id]} onCheckedChange={(checked) => setSelected((prev) => ({ ...prev, [item.id]: !!checked }))} className="absolute left-3 top-3 z-10 bg-background shadow"/>{image ? <img src={image} alt={item.title} loading="lazy" className="h-full w-full object-contain p-5 transition-transform duration-300 group-hover:scale-[1.03]"/> : <div className="flex h-full items-center justify-center text-sm text-muted-foreground">Sem imagem disponível</div>}</div><div className="space-y-4 p-4"><div><div className="mb-2 flex items-center justify-between gap-2"><Badge variant="outline" className="text-[10px]">{item.verified_item ? "Anúncio confirmado" : "Anúncio público"}</Badge>{hasSales && <span className="flex items-center gap-1 text-xs font-semibold"><ShoppingCart className="h-3.5 w-3.5"/>{item.sold_quantity} venda(s)</span>}</div><p className="line-clamp-2 min-h-10 text-sm font-semibold leading-5">{item.title}</p><p className="mt-2 font-display text-2xl font-extrabold">{item.price_cents == null ? <span className="text-base text-muted-foreground">Preço não disponível</span> : formatBRL(item.price_cents)}</p>{!item.verified_item && <p className="mt-2 text-[11px] text-muted-foreground">Métricas de vendas não confirmadas; por isso não são exibidas.</p>}</div><div className="flex flex-wrap gap-1.5">{(item.seller||item.seller_id)&&<Button type="button" size="sm" variant="outline" className="h-7 rounded-full px-2 text-[10px]" onClick={()=>searchSellerFromCard(item)}><Store className="mr-1 h-3 w-3"/>{item.seller??`Vendedor ${item.seller_id}`}</Button>}<Badge variant="secondary" className="text-[10px]">{conditionLabel(item.condition)}</Badge>{item.status&&<Badge variant="secondary" className="text-[10px]">{listingStatusLabel(item.status)}</Badge>}</div>{item.available_quantity!=null&&<div className="rounded-xl bg-muted/50 p-3 text-xs"><b>Estoque:</b> {item.available_quantity}</div>}<div className="flex items-center justify-between gap-2 rounded-xl bg-muted/50 px-3 py-2"><span className="font-mono text-[10px] text-muted-foreground">{item.id}</span><Button size="icon" variant="ghost" className="h-7 w-7" onClick={async()=>{await navigator.clipboard.writeText(item.id);toast.success("Código copiado")}} aria-label="Copiar código MLB"><Clipboard className="h-3.5 w-3.5"/></Button></div><Button className="w-full rounded-xl bg-yellow-400 text-black hover:bg-yellow-300" disabled={item.price_cents == null} onClick={() => setPreview({ item, action: "copy" })}><Files className="mr-2 h-4 w-4"/>Copiar anúncio<ArrowRight className="ml-auto h-4 w-4"/></Button><div className="grid grid-cols-2 gap-2"><Button size="sm" variant="outline" onClick={() => openDetails.mutate(item)}><Eye className="h-3.5 w-3.5"/>Ver detalhes</Button><Button size="sm" variant="outline" disabled={editOne.isPending || item.price_cents == null} onClick={() => editOne.mutate(item)}><Edit className="h-3.5 w-3.5"/>Editar antes</Button></div>{item.permalink&&<Button size="sm" variant="ghost" className="w-full" asChild><a href={item.permalink} target="_blank" rel="noopener noreferrer"><ExternalLink className="mr-2 h-3.5 w-3.5"/>Ver no Mercado Livre</a></Button>}</div></CardContent></Card>; })}</div>
      </>}
