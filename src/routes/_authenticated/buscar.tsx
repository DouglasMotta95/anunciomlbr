    ...
    <AppShell title="Buscar e copiar" description="Pesquise anúncios reais do Mercado Livre, compare desempenho e crie uma cópia editável.">
      <section className="overflow-hidden rounded-3xl border border-yellow-400/30 bg-gradient-to-br from-yellow-400/15 via-card to-card shadow-sm">
        <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1.35fr_.65fr] lg:items-center">
          <div>
            <div className="flex flex-wrap items-center gap-2"><Badge className="bg-yellow-400 text-black hover:bg-yellow-400">INTEGRAÇÃO MERCADO LIVRE</Badge><Badge variant="outline">BUSCA INTELIGENTE</Badge></div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Busque por produto, código MLB, link ou vendedor. Quando a API informa vendas, os resultados são ordenados do mais vendido para o menos vendido entre os resultados encontrados.</p>
            <form className="mt-5 grid gap-2 sm:grid-cols-[minmax(0,1fr)_150px_auto]" onSubmit={(event) => { event.preventDefault(); if (query.trim().length > 1) runSearch.mutate({ raw: query.trim() }); }}>
              <div className="relative"><Search className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground"/><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Ex.: iPhone 15, MLB123..., link ou @vendedor" className="h-14 rounded-2xl bg-background/95 pl-12 text-base shadow-sm"/></div>
            ...
            <p className="mt-2 text-xs text-muted-foreground">Consultas maiores são paginadas e podem levar um pouco mais de tempo. Descrições completas são carregadas somente quando você abre os detalhes ou copia o anúncio.</p>
          </div>
          <div className="grid gap-2 rounded-2xl border border-yellow-400/20 bg-background/75 p-4 backdrop-blur">
            ...
            <div className="grid gap-2 text-sm"><span className="rounded-xl bg-muted/60 px-3 py-2">Produto ou palavra-chave</span><span className="rounded-xl bg-muted/60 px-3 py-2">ID do anúncio · MLB123...</span><span className="rounded-xl bg-muted/60 px-3 py-2">Link do Mercado Livre</span><span className="rounded-xl bg-muted/60 px-3 py-2">Vendedor · @NICKNAME ou ID</span></div>
          </div>
        </div>
      </section>
