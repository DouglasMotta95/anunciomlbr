...
import { ArrowRight, Clipboard, Download, Edit, ExternalLink, Eye, Files, Loader2, Search, SearchX, ShoppingCart, Sparkles, Trophy } from "lucide-react";
...
type DisplayItem = MlItem & { description?: string | null; source_kind?: "marketplace" | "catalog_offer" };
...
function SearchPage() {
  ...
  const createOne = useMutation({
    mutationFn: async ({ item, action }: { item: DisplayItem; action: PreviewAction }) => {
      if (item.price_cents == null) throw new Error("Este anúncio não retornou preço suficiente para criar uma cópia fiel.");
  ...
  const editOne = useMutation({
    mutationFn: async (item: DisplayItem) => {
      if (item.price_cents == null) throw new Error("Preço não disponível para copiar este anúncio.");
  ...
  return (
    <AppShell title="Buscar e copiar" description="Pesquise anúncios reais do Mercado Livre, compare desempenho e crie uma cópia editável.">
      <section className="overflow-hidden rounded-3xl border border-yellow-400/30 bg-gradient-to-br from-yellow-400/15 via-card to-card shadow-sm">
        <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1.35fr_.65fr] lg:items-center">
          <div>
            <div className="flex flex-wrap items-center gap-2"><Badge className="bg-yellow-400 text-black hover:bg-yellow-400">MERCADO LIVRE</Badge><Badge variant="outline">BUSCA INTELIGENTE</Badge></div>
            <h2 className="mt-3 max-w-2xl text-2xl font-extrabold tracking-tight sm:text-3xl">Encontre anúncios fortes para usar como referência.</h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Busque por produto, código MLB, link ou vendedor. Quando a API informa vendas, os resultados são ordenados do mais vendido para o menos vendido.</p>
            <form className="mt-5 flex flex-col gap-2 sm:flex-row" onSubmit={(event) => { event.preventDefault(); if (query.trim().length > 1) runSearch.mutate(query.trim()); }}>
              ...
              <Button type="submit" size="lg" className="h-14 rounded-2xl bg-yellow-400 px-8 text-black hover:bg-yellow-300" disabled={runSearch.isPending || query.trim().length < 2}>{runSearch.isPending ? <Loader2 className="h-4 w-4 animate-spin"/> : <Search className="h-4 w-4"/>}{runSearch.isPending ? "Buscando..." : "Buscar no Mercado Livre"}</Button>
            </form>
          </div>
      ...
      {runSearch.isPending && <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className="h-[430px] rounded-3xl"/>)}</div>}
      {!runSearch.isPending && searched && !items.length && <div className="mt-6 flex flex-col items-center gap-2 rounded-3xl border border-dashed bg-muted/20 p-10 text-center text-muted-foreground"><SearchX className="h-9 w-9"/><p className="font-semibold text-foreground">Nenhum anúncio completo encontrado</p><p className="max-w-lg text-sm">Tente uma descrição mais específica, um vendedor, link ou código MLB.</p></div>}

      {!!items.length && <>
        <div className="mt-6 flex flex-col gap-3 rounded-2xl border bg-card px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><Checkbox checked={selectedIds.length === items.length} onCheckedChange={(checked) => setSelected(checked ? Object.fromEntries(items.map((item) => [item.id, true])) : {})} id="select-all"/><label htmlFor="select-all" className="cursor-pointer text-sm font-semibold">{items.length} anúncio(s) encontrado(s)</label></div><p className="text-xs text-muted-foreground">Ranking usa a quantidade de vendas quando o Mercado Livre fornece esse dado.</p></div>
        <div className="mt-3 grid gap-4 pb-28 sm:grid-cols-2 xl:grid-cols-3">{items.map((item, index) => { const image = getProductImage(item); const hasSales = item.sold_quantity != null; return <Card key={item.id} className="group overflow-hidden rounded-3xl border-border/70 transition-all duration-300 hover:-translate-y-1 hover:border-yellow-400/60 hover:shadow-lg"><CardContent className="p-0"><div className="relative aspect-[4/3] overflow-hidden border-b bg-white"><Checkbox checked={!!selected[item.id]} onCheckedChange={(checked) => setSelected((prev) => ({ ...prev, [item.id]: !!checked }))} className="absolute left-3 top-3 z-10 bg-background shadow"/>{index === 0 && hasSales && <Badge className="absolute right-3 top-3 z-10 bg-yellow-400 text-black"><Trophy className="mr-1 h-3 w-3"/>Mais vendido</Badge>}{image ? <img src={image} alt={item.title} loading="lazy" className="h-full w-full object-contain p-5 transition-transform duration-300 group-hover:scale-[1.03]"/> : <div className="flex h-full items-center justify-center text-sm text-muted-foreground">Sem imagem disponível</div>}</div><div className="space-y-4 p-4"><div><div className="mb-2 flex items-center justify-between gap-2"><Badge className="bg-yellow-400 text-[10px] text-black hover:bg-yellow-400">Mercado Livre</Badge>{hasSales && <span className="flex items-center gap-1 text-xs font-semibold"><ShoppingCart className="h-3.5 w-3.5"/>{item.sold_quantity} venda(s)</span>}</div><p className="line-clamp-2 min-h-10 text-sm font-semibold leading-5">{item.title}</p><p className="mt-2 font-display text-2xl font-extrabold">{item.price_cents == null ? <span className="text-base text-muted-foreground">Preço não disponível</span> : formatBRL(item.price_cents)}</p></div><div className="flex flex-wrap gap-1.5">{item.seller&&<Badge variant="outline" className="text-[10px]">Vendedor: {item.seller}</Badge>}<Badge variant="secondary" className="text-[10px]">{conditionLabel(item.condition)}</Badge>{item.status&&<Badge variant="secondary" className="text-[10px]">{listingStatusLabel(item.status)}</Badge>}</div><div className="grid grid-cols-2 gap-2 rounded-xl bg-muted/50 p-3 text-xs"><span><b>Estoque:</b> {item.available_quantity ?? "—"}</span><span><b>Ranking:</b> {hasSales ? `#${index + 1}` : "—"}</span></div><div className="flex items-center justify-between gap-2 rounded-xl bg-muted/50 px-3 py-2"><span className="font-mono text-[10px] text-muted-foreground">{item.id}</span><Button size="icon" variant="ghost" className="h-7 w-7" onClick={async()=>{await navigator.clipboard.writeText(item.id);toast.success("Código copiado")}} aria-label="Copiar código MLB"><Clipboard className="h-3.5 w-3.5"/></Button></div><Button className="w-full rounded-xl bg-yellow-400 text-black hover:bg-yellow-300" disabled={item.price_cents == null} onClick={() => setPreview({ item, action: "copy" })}><Files className="mr-2 h-4 w-4"/>Copiar anúncio<ArrowRight className="ml-auto h-4 w-4"/></Button><div className="grid grid-cols-2 gap-2"><Button size="sm" variant="outline" onClick={() => setDetails(item)}><Eye className="h-3.5 w-3.5"/>Ver detalhes</Button><Button size="sm" variant="outline" disabled={editOne.isPending || item.price_cents == null} onClick={() => editOne.mutate(item)}><Edit className="h-3.5 w-3.5"/>Editar antes</Button></div></div></CardContent></Card>; })}</div>
      </>}
      ...
      <Dialog open={!!details} onOpenChange={(open) => !open && setDetails(null)}><DialogContent className="max-w-2xl">{details&&<><DialogHeader><DialogTitle>Detalhes do anúncio</DialogTitle><DialogDescription>Informações retornadas pelo Mercado Livre para este anúncio.</DialogDescription></DialogHeader><div className="grid gap-5 sm:grid-cols-[190px_1fr]">{getProductImage(details)?<img src={getProductImage(details)??undefined} alt={details.title} className="aspect-square w-full rounded-xl border bg-white object-contain p-3"/>:<div className="flex aspect-square items-center justify-center rounded-xl bg-muted text-xs text-muted-foreground">Sem imagem</div>}<div className="space-y-3"><p className="font-semibold leading-5">{details.title}</p><p className="text-2xl font-extrabold">{details.price_cents == null ? "Preço não disponível" : formatBRL(details.price_cents)}</p><div className="flex flex-wrap gap-1.5">{details.seller&&<Badge variant="outline">{details.seller}</Badge>}<Badge variant="secondary">{conditionLabel(details.condition)}</Badge>{details.sold_quantity!=null&&<Badge className="bg-yellow-400 text-black">{details.sold_quantity} vendas</Badge>}</div><p className="text-xs text-muted-foreground">ID {details.id} · Estoque {details.available_quantity ?? "—"}</p></div></div><div className="rounded-2xl border bg-muted/20 p-4"><p className="mb-2 text-sm font-bold">Descrição do anúncio</p><p className="max-h-56 overflow-y-auto whitespace-pre-wrap text-sm leading-6 text-muted-foreground">{details.description || "O Mercado Livre não disponibilizou a descrição deste anúncio para esta consulta."}</p></div><DialogFooter>{details.permalink&&<Button variant="outline" asChild><a href={details.permalink} target="_blank" rel="noreferrer"><ExternalLink className="h-4 w-4"/>Ver original</a></Button>}<Button disabled={details.price_cents == null} onClick={() => { setDetails(null); setPreview({ item: details, action: "copy" }); }} className="bg-yellow-400 text-black hover:bg-yellow-300"><Files className="h-4 w-4"/>Copiar anúncio</Button></DialogFooter></>}</DialogContent></Dialog>

      <Dialog open={!!preview} onOpenChange={(open) => !open && setPreview(null)}><DialogContent className="max-w-xl">{preview&&<><DialogHeader><DialogTitle>Confirmar cópia do anúncio</DialogTitle><DialogDescription>Confira os dados antes de criar seu novo rascunho. A cópia consome 1 unidade da sua franquia de criações.</DialogDescription></DialogHeader><div className="grid gap-4 sm:grid-cols-[160px_1fr]">{getProductImage(preview.item)?<img src={getProductImage(preview.item)??undefined} alt={preview.item.title} className="aspect-square w-full rounded-xl border bg-white object-contain p-2"/>:<div className="flex aspect-square items-center justify-center rounded-xl bg-muted text-xs text-muted-foreground">Sem imagem</div>}<div className="space-y-2"><p className="font-semibold leading-5">{preview.item.title}</p><p className="text-2xl font-extrabold">{preview.item.price_cents == null ? "Preço não disponível" : formatBRL(preview.item.price_cents)}</p><div className="flex flex-wrap gap-1.5">{preview.item.seller&&<Badge variant="outline">{preview.item.seller}</Badge>}{preview.item.sold_quantity!=null&&<Badge className="bg-yellow-400 text-black">{preview.item.sold_quantity} vendas</Badge>}</div><p className="text-xs text-muted-foreground">Estoque retornado: {preview.item.available_quantity ?? "—"} · ID {preview.item.id}</p></div></div><DialogFooter><Button variant="outline" onClick={() => setPreview(null)}>Cancelar</Button><Button disabled={createOne.isPending || preview.item.price_cents == null} onClick={() => createOne.mutate(preview)} className="bg-yellow-400 text-black hover:bg-yellow-300">{createOne.isPending?<Loader2 className="h-4 w-4 animate-spin"/>:<Files className="h-4 w-4"/>}Criar cópia</Button></DialogFooter></>}</DialogContent></Dialog>
