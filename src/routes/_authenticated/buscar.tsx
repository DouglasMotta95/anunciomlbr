...
function SearchPage() {
  ...
  return (
    <AppShell title="Buscar e copiar" description="Pesquise anúncios do marketplace, confira dados confirmados e crie uma cópia editável.">
      <section className="overflow-hidden rounded-3xl border border-yellow-400/30 bg-gradient-to-br from-yellow-400/15 via-card to-card shadow-sm">
        <div className="grid gap-6 p-5 sm:p-7 lg:grid-cols-[1.35fr_.65fr] lg:items-center">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">A API oficial é sempre a primeira fonte. Se ela não devolver nenhum resultado confirmado, o sistema consulta a busca pública do Mercado Livre como fallback, sem inventar códigos ou links.</p>
            ...
            <p className="mt-2 text-xs text-muted-foreground">Vendas só são exibidas quando o Mercado Livre confirma o número pela API. Resultados do site público são identificados como públicos.</p>
          </div>
      ...
      {!!items.length && <>
        <div className="mt-6 flex flex-col gap-3 rounded-2xl border bg-card px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><Checkbox checked={selectedIds.length === items.length} onCheckedChange={(checked) => setSelected(checked ? Object.fromEntries(items.map((item) => [item.id, true])) : {})} id="select-all"/><label htmlFor="select-all" className="cursor-pointer text-sm font-semibold">{items.length} anúncio(s) encontrado(s)</label></div><div className="flex items-center gap-2"><p className="text-xs text-muted-foreground">API oficial primeiro · site público apenas como fallback.</p>{marketplaceSearchUrl&&<Button size="sm" variant="outline" asChild><a href={marketplaceSearchUrl} target="_blank" rel="noopener noreferrer"><ExternalLink className="mr-1.5 h-3.5 w-3.5"/>Comparar no ML</a></Button>}</div></div>
        <div className="mt-3 grid gap-4 pb-28 sm:grid-cols-2 xl:grid-cols-3">{items.map((item) => {
          ...
          return <Card key={item.id} className="group overflow-hidden rounded-3xl border-border/70 transition-all duration-300 hover:-translate-y-1 hover:border-yellow-400/60 hover:shadow-lg"><CardContent className="p-0">
            <div className="relative aspect-[4/3] overflow-hidden border-b bg-white"><Checkbox checked={!!selected[item.id]} onCheckedChange={(checked) => setSelected((prev) => ({ ...prev, [item.id]: !!checked }))} className="absolute left-3 top-3 z-10 bg-background shadow"/>{image ? <img src={image} alt={item.title} loading="lazy" className="h-full w-full object-contain p-5 transition-transform duration-300 group-hover:scale-[1.03]"/> : <div className="flex h-full items-center justify-center text-sm text-muted-foreground">Sem imagem disponível</div>}</div>
            <div className="space-y-4 p-4">
              <div><div className="mb-2 flex items-center justify-between gap-2"><div className="flex flex-wrap gap-1.5"><Badge variant="outline" className="text-[10px]">{item.verified_item ? "Anúncio confirmado" : "Anúncio público"}</Badge>{item.source_kind === "catalog_offer" && <Badge className="bg-sky-500/15 text-sky-700 hover:bg-sky-500/15">Oferta de catálogo</Badge>}</div>{hasSales && <span className="flex items-center gap-1 text-xs font-semibold"><ShoppingCart className="h-3.5 w-3.5"/>{item.sold_quantity} venda(s)</span>}</div><p className="line-clamp-2 min-h-10 text-sm font-semibold leading-5">{item.title}</p><p className="mt-2 font-display text-2xl font-extrabold">{item.price_cents == null ? <span className="text-base text-muted-foreground">Preço não disponível</span> : formatBRL(item.price_cents)}</p>{!item.verified_item && <p className="mt-2 text-[11px] text-muted-foreground">Dados extraídos da página pública porque a API não confirmou ofertas para esta busca.</p>}</div>
              <div className="flex flex-wrap gap-1.5">{(item.seller||item.seller_id)&&<Button type="button" size="sm" variant="outline" className="h-7 rounded-full px-2 text-[10px]" onClick={()=>searchSellerFromCard(item)}><Store className="mr-1 h-3 w-3"/>{item.seller??`Vendedor ${item.seller_id}`}</Button>}<Badge variant="secondary" className="text-[10px]">{conditionLabel(item.condition)}</Badge>{item.status&&<Badge variant="secondary" className="text-[10px]">{listingStatusLabel(item.status)}</Badge>}</div>
              ...
              <div className="grid grid-cols-2 gap-2"><Button size="sm" variant="outline" disabled={!item.permalink} asChild={!!item.permalink}>{item.permalink ? <a href={item.permalink} target="_blank" rel="noopener noreferrer"><ExternalLink className="h-3.5 w-3.5"/>Ver anúncio</a> : <><ExternalLink className="h-3.5 w-3.5"/>Ver anúncio</>}</Button><Button size="sm" className="bg-yellow-400 text-black hover:bg-yellow-300" disabled={createOne.isPending || item.price_cents == null} onClick={() => setPreview({ item, action: "copy" })}><Files className="h-3.5 w-3.5"/>Duplicar anúncio</Button></div>
              <div className="grid grid-cols-2 gap-2"><Button size="sm" variant="ghost" onClick={() => openDetails.mutate(item)}><Eye className="h-3.5 w-3.5"/>Ver detalhes</Button><Button size="sm" variant="ghost" disabled={editOne.isPending || item.price_cents == null} onClick={() => editOne.mutate(item)}><Edit className="h-3.5 w-3.5"/>Editar antes</Button></div>
            </div>
          </CardContent></Card>;
        })}</div>
      </>}
      ...
      <Dialog open={!!details} onOpenChange={(open) => { if(!open){setDetails(null);setDetailsReason(null);} }}><DialogContent className="max-w-2xl">{details&&<><DialogHeader><DialogTitle>Detalhes do anúncio</DialogTitle><DialogDescription>Informações retornadas pelo Mercado Livre para esta oferta.</DialogDescription></DialogHeader>{openDetails.isPending?<div className="space-y-4"><Skeleton className="h-48 w-full rounded-2xl"/><Skeleton className="h-28 w-full rounded-2xl"/></div>:<><div className="grid gap-5 sm:grid-cols-[190px_1fr]">{getProductImage(details)?<img src={getProductImage(details)??undefined} alt={details.title} className="aspect-square w-full rounded-xl border bg-white object-contain p-3"/>:<div className="flex aspect-square items-center justify-center rounded-xl bg-muted text-xs text-muted-foreground">Sem imagem</div>}<div className="space-y-3"><p className="font-semibold leading-5">{details.title}</p><p className="text-2xl font-extrabold">{details.price_cents == null ? "Preço não disponível" : formatBRL(details.price_cents)}</p><div className="flex flex-wrap gap-1.5">{details.seller&&<Badge variant="outline">{details.seller}</Badge>}{details.source_kind === "catalog_offer"&&<Badge variant="secondary">Oferta de catálogo</Badge>}<Badge variant="secondary">{conditionLabel(details.condition)}</Badge>{details.verified_item&&details.sold_quantity!=null&&<Badge className="bg-yellow-400 text-black">{details.sold_quantity} vendas</Badge>}</div><p className="text-xs text-muted-foreground">ID {details.id}{details.available_quantity!=null?` · Estoque ${details.available_quantity}`:""}</p></div></div><div className="rounded-2xl border bg-muted/20 p-4"><p className="mb-2 text-sm font-bold">Descrição do anúncio</p>{details.description?<p className="max-h-56 overflow-y-auto whitespace-pre-wrap text-sm leading-6 text-muted-foreground">{details.description}</p>:<p className="text-sm text-muted-foreground">{detailsReason??"Este anúncio não possui descrição disponível."}</p>}</div></>}<DialogFooter>{details.permalink&&<Button variant="outline" asChild><a href={details.permalink} target="_blank" rel="noopener noreferrer"><ExternalLink className="h-4 w-4"/>Ver anúncio</a></Button>}<Button disabled={details.price_cents == null||openDetails.isPending} onClick={() => { setDetails(null); setPreview({ item: details, action: "copy" }); }} className="bg-yellow-400 text-black hover:bg-yellow-300"><Files className="h-4 w-4"/>Duplicar anúncio</Button></DialogFooter></>}</DialogContent></Dialog>

      <Dialog open={!!preview} onOpenChange={(open) => !open && setPreview(null)}><DialogContent className="max-w-xl">{preview&&<><DialogHeader><DialogTitle>Confirmar duplicação do anúncio</DialogTitle><DialogDescription>O sistema cria um novo rascunho editável no painel. A confirmação só aparece depois que o banco retorna o ID real da cópia criada.</DialogDescription></DialogHeader><div className="grid gap-4 sm:grid-cols-[160px_1fr]">{getProductImage(preview.item)?<img src={getProductImage(preview.item)??undefined} alt={preview.item.title} className="aspect-square w-full rounded-xl border bg-white object-contain p-2"/>:<div className="flex aspect-square items-center justify-center rounded-xl bg-muted text-xs text-muted-foreground">Sem imagem</div>}<div className="space-y-2"><p className="font-semibold leading-5">{preview.item.title}</p><p className="text-2xl font-extrabold">{preview.item.price_cents == null ? "Preço não disponível" : formatBRL(preview.item.price_cents)}</p><div className="flex flex-wrap gap-1.5">{preview.item.seller&&<Badge variant="outline">{preview.item.seller}</Badge>}{preview.item.source_kind === "catalog_offer"&&<Badge variant="secondary">Oferta de catálogo</Badge>}{preview.item.verified_item&&preview.item.sold_quantity!=null&&<Badge className="bg-yellow-400 text-black">{preview.item.sold_quantity} vendas</Badge>}</div><p className="text-xs text-muted-foreground">{preview.item.available_quantity!=null?`Estoque retornado: ${preview.item.available_quantity}`:"Estoque não informado; o novo rascunho começará com 1 unidade para edição."} · ID {preview.item.id}</p></div></div><DialogFooter><Button variant="outline" onClick={() => setPreview(null)}>Cancelar</Button><Button disabled={createOne.isPending || preview.item.price_cents == null} onClick={() => createOne.mutate(preview)} className="bg-yellow-400 text-black hover:bg-yellow-300">{createOne.isPending?<Loader2 className="h-4 w-4 animate-spin"/>:<Files className="h-4 w-4"/>}{createOne.isPending?"Criando duplicação...":"Duplicar anúncio"}</Button></DialogFooter></>}</DialogContent></Dialog>
