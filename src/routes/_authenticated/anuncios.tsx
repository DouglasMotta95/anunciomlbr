...
import { Archive, AlertTriangle, Copy, Download, Files, Loader2, Pause, Play, PlusCircle, Sparkles, Tag, Trash2 } from "lucide-react";
...
function ListingsPage() {
  ...
  const variants = useMutation({
    ...
    onSuccess: (result) => { void queryClient.invalidateQueries({ queryKey: ["listings"] }); void queryClient.invalidateQueries({ queryKey: ["ad-quota"] }); toast.success(`${result.created.length} variações criadas`, { description: "Cada rascunho recebeu um título diferente gerado pela IA." }); setSelected({}); },
  ...
  return <AppShell title="Meus anúncios" description="Status, saúde do anúncio, otimizações de IA e ações em massa em um só lugar." actions={<><Button asChild variant="outline" size="sm"><Link to="/buscar"><Tag className="mr-1.5 h-3.5 w-3.5"/>Buscar no ML</Link></Button><Button asChild size="sm"><Link to="/editor/$id" params={{ id: "novo" }}><PlusCircle className="mr-1.5 h-3.5 w-3.5"/>Novo anúncio</Link></Button></>}>
    ...
    <div className="mt-4">{isLoading ? <div className="grid gap-3">{Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-28 rounded-2xl"/>)}</div> : filtered.length === 0 ? <Card><CardContent className="flex flex-col items-center gap-3 py-14 text-center"><Tag className="h-8 w-8 text-muted-foreground"/><p className="font-display text-lg font-bold">Nenhum anúncio nesta visão</p><p className="max-w-md text-sm text-muted-foreground">Busque anúncios no Mercado Livre, clone estruturas ou crie um anúncio do zero.</p><div className="flex gap-2"><Button asChild><Link to="/buscar">Buscar e clonar</Link></Button><Button variant="outline" asChild><Link to="/editor/$id" params={{ id: "novo" }}>Criar do zero</Link></Button></div></CardContent></Card> : <>
    ...
    {selectedIds.length > 0&&<div className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-card/95 backdrop-blur"><div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6"><div><p className="text-sm font-semibold">{selectedIds.length} anúncio(s) selecionado(s)</p><p className="text-xs text-muted-foreground">Ações em massa: duplicar, otimizar, ativar, pausar, arquivar ou excluir.</p></div><div className="flex flex-wrap gap-2">{selectedIds.length === 1&&<><Button size="sm" variant="secondary" disabled={variants.isPending} onClick={() => variants.mutate(5)}>{variants.isPending&&variants.variables===5?<Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin"/>:<Sparkles className="mr-1.5 h-3.5 w-3.5"/>}5 variações IA</Button><Button size="sm" variant="secondary" disabled={variants.isPending} onClick={() => variants.mutate(10)}>{variants.isPending&&variants.variables===10?<Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin"/>:<Sparkles className="mr-1.5 h-3.5 w-3.5"/>}10 variações IA</Button></>}<Button size="sm" onClick={() => startBulk("duplicate")}><Files className="mr-1.5 h-3.5 w-3.5"/>Duplicar</Button><Button size="sm" variant="secondary" onClick={() => startBulk("optimize")}><Sparkles className="mr-1.5 h-3.5 w-3.5"/>Otimizar</Button><Button size="sm" variant="outline" onClick={() => startBulk("activate")}><Play className="mr-1.5 h-3.5 w-3.5"/>Ativar</Button><Button size="sm" variant="outline" onClick={() => startBulk("pause")}><Pause className="mr-1.5 h-3.5 w-3.5"/>Pausar</Button><Button size="sm" variant="outline" onClick={() => startBulk("archive")}><Archive className="mr-1.5 h-3.5 w-3.5"/>Arquivar</Button><Button size="sm" variant="outline" onClick={copyCodes}><Copy className="mr-1.5 h-3.5 w-3.5"/>Códigos</Button><Button size="sm" variant="outline" onClick={exportCsv}><Download className="mr-1.5 h-3.5 w-3.5"/>Exportar</Button><Button size="sm" variant="destructive" onClick={() => startBulk("delete")}><Trash2 className="mr-1.5 h-3.5 w-3.5"/>Excluir</Button></div></div></div>}
