...
import { Archive, AlertTriangle, CheckCircle2, Copy, Download, ExternalLink, Files, ImagePlus, Loader2, Pause, Play, PlusCircle, Sparkles, Tag, Trash2 } from "lucide-react";
...
function listingStatusLabel(listing: any) {
  if (listing.published_ml_id && listing.status === "draft") return "Publicado · status pendente";
  return STATUS_LABEL[listing.status] ?? "Status desconhecido";
}
...
function ListingsPage() {
  ...
  const filtered = useMemo(() => (listings as Listing[]).filter((listing: any) => {
    ...
    if (tab === "draft") return listing.status === "draft" && !listing.published_ml_id;
  ...
  const copyCodes = async () => {
    const codes = selectedListings.map((listing) => listing.published_ml_id || listing.source_ml_id || listing.id);
    await navigator.clipboard.writeText(codes.join("\n"));
    toast.success(`✓ ${codes.length} códigos copiados`);
  };
  const exportCsv = () => {
    const header = "id,titulo,preco,status,estoque,score\n";
    const rows = selectedListings.map((listing) => [listing.id, String(listing.title).replace(/,/g, " "), listing.price_cents ?? "", listingStatusLabel(listing), listing.stock, listing.ai_score ?? ""].join(",")).join("\n");
    ...
    <div className="mt-4">
      {isLoading ? <div className="grid gap-3">{Array.from({ length: 4 }).map((_, index) => <Skeleton key={index} className="h-28 rounded-2xl" />)}</div> : filtered.length === 0 ? <Card><CardContent className="flex flex-col items-center gap-3 py-14 text-center"><Tag className="h-8 w-8 text-muted-foreground" /><p className="font-display text-lg font-bold">Nenhum anúncio nesta visão</p><p className="max-w-md text-sm text-muted-foreground">Busque anúncios no Mercado Livre, copie estruturas ou crie um anúncio do zero.</p><div className="flex gap-2"><Button asChild><Link to="/buscar">Buscar e copiar</Link></Button><Button variant="outline" asChild><Link to="/editor/$id" params={{ id: "novo" }}>Criar do zero</Link></Button></div></CardContent></Card> : <>
        ...
        <div className="grid gap-3 pb-24">{pageItems.map((listing) => {
          ...
          const publicationPermalink = listing.published_permalink || (imported ? listing.source_permalink : null);
          return <Card key={listing.id} className={issues.length ? "border-amber-500/25" : ""}><CardContent className="space-y-3 pt-6"><div className="flex flex-wrap items-center gap-4"><Checkbox checked={!!selected[listing.id]} onCheckedChange={(checked) => setSelected((prev) => ({ ...prev, [listing.id]: !!checked }))} />{getProductImage(listing.images) ? <img src={getProductImage(listing.images) ?? undefined} alt={listing.title} loading="lazy" className="h-16 w-16 shrink-0 rounded-md object-cover" /> : <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-md bg-muted text-[10px] text-muted-foreground">Sem imagem</div>}<div className="min-w-[240px] flex-1"><div className="flex flex-wrap items-center gap-2"><span className="text-sm font-semibold">{listing.title}</span><Badge variant={listing.status === "active" ? "default" : "outline"}>{listingStatusLabel(listing)}</Badge><Badge variant="outline">{imported ? "Importado do Mercado Livre" : "Criado no ANÚNCIO ML"}</Badge>{optimized && <Badge variant="secondary"><Sparkles className="mr-1 h-3 w-3" />Otimizado</Badge>}{listing.stock <= 2 && <Badge variant="destructive">Estoque baixo</Badge>}</div><p className="mt-1 text-xs text-muted-foreground">{formatBRL(listing.price_cents)} · estoque {listing.stock} · adicionado em {formatDate(listing.created_at)}</p></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" asChild><Link to="/editor/$id" params={{ id: listing.id }}>Editar</Link></Button><Button size="sm" variant="secondary" onClick={() => void startBulk("duplicate", [listing])} title="Cria um novo rascunho e usa 1 da franquia"><Files className="mr-1.5 h-3.5 w-3.5" />Duplicar</Button>{listing.source_permalink && <Button size="sm" variant="outline" asChild><a href={listing.source_permalink} target="_blank" rel="noopener noreferrer"><ExternalLink className="mr-1.5 h-3.5 w-3.5" />Ver anúncio original</a></Button>}<Button size="icon" variant="ghost" onClick={() => remove.mutate(listing.id)} aria-label="Excluir anúncio"><Trash2 className="h-4 w-4" /></Button></div></div>
          {listing.published_ml_id ? <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-emerald-500/30 bg-emerald-500/[.06] p-3"><div className="flex items-start gap-2"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-700"/><div><p className="text-xs font-semibold text-emerald-800">Publicado no Mercado Livre</p><p className="mt-1 font-mono text-xs text-muted-foreground">{listing.published_ml_id}</p></div></div>{publicationPermalink ? <Button size="sm" variant="outline" asChild><a href={publicationPermalink} target="_blank" rel="noopener noreferrer"><ExternalLink className="mr-1.5 h-3.5 w-3.5"/>{imported ? "Abrir anúncio no Mercado Livre" : "Abrir publicação criada"}</a></Button> : <p className="text-xs text-muted-foreground">MLB registrado; permalink ainda não confirmado.</p>}</div> : <div className="rounded-xl border bg-muted/25 p-3 text-xs text-muted-foreground"><strong className="text-foreground">Rascunho — ainda não publicado.</strong> Duplicar ou copiar cria um rascunho interno; isso não significa publicação no Mercado Livre.</div>}
          {issues.length > 0 ? <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-500/25 bg-amber-500/[.05] p-3"><div className="flex items-start gap-2"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-600" /><div><p className="text-xs font-semibold">Saúde do anúncio: {issues.length} ponto(s) para corrigir</p><p className="mt-1 text-xs text-muted-foreground">{issues.join(" · ")}</p></div></div><div className="flex flex-wrap gap-2">{imageIssue && <Button size="sm" asChild><Link to="/editor/$id" params={{ id: listing.id }}><ImagePlus className="mr-1.5 h-3.5 w-3.5" />Gerar imagem com IA</Link></Button>}<Button size="sm" variant="outline" asChild><Link to="/editor/$id" params={{ id: listing.id }}>{imageIssue ? "Outros ajustes" : "Corrigir no editor"}</Link></Button></div></div> : <div className="rounded-xl bg-emerald-500/[.06] p-3 text-xs font-medium text-emerald-700">Saúde básica em dia: nenhum alerta estrutural detectado.</div>}</CardContent></Card>;
        })}</div>
