...
function Commercial(){
 ...
 return <AdminLayout activeSection="pagamentos" onSectionChange={()=>{}}><div className="space-y-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-display text-2xl font-extrabold">Assistente comercial</h2><p className="text-sm text-muted-foreground">Recupere pagamentos pendentes e recusados com mensagens geradas pela IA e revisão humana antes do envio.</p></div><Button variant="outline" onClick={()=>refetch()}><RefreshCcw className="mr-2 h-4 w-4"/>Atualizar</Button></div><div className="grid gap-4">{isLoading?<Card><CardContent className="p-8 text-sm text-muted-foreground">Carregando oportunidades...</CardContent></Card>:(data??[]).length===0?<Card><CardContent className="p-8 text-center text-sm text-muted-foreground">Nenhum pagamento pendente ou recusado para recuperar.</CardContent></Card>:(data??[]).map((p:any)=>{const message=messages[p.id];return <Card key={p.id}><CardHeader className="flex flex-row items-start justify-between gap-3"><div><CardTitle className="text-base">{p.email??'Cliente sem e-mail'}</CardTitle><p className="mt-1 text-xs text-muted-foreground">{p.plan??'Plano não informado'} · {formatBRL(p.amount_cents)} · {formatDateTime(p.created_at)}</p></div><Badge variant={p.status==='rejected'?'destructive':'secondary'}>{p.status==='rejected'?'Recusado':'Pendente'}</Badge></CardHeader><CardContent className="space-y-3">{message?<div className="rounded-xl border bg-muted/30 p-4"><div className="mb-2 flex items-center gap-2 text-sm font-semibold"><Bot className="h-4 w-4 text-primary"/>Mensagem sugerida</div><p className="whitespace-pre-wrap text-sm">{message}</p><div className="mt-3 flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={()=>{void navigator.clipboard.writeText(message);toast.success('Mensagem copiada')}}><Copy className="mr-2 h-4 w-4"/>Copiar</Button><Button size="sm" onClick={()=>action.mutate({payment_id:p.id,action:'contacted'})}><MessageCircle className="mr-2 h-4 w-4"/>Marcar contatado</Button><Button size="sm" variant="outline" onClick={()=>action.mutate({payment_id:p.id,action:'resolved'})}><Check className="mr-2 h-4 w-4"/>Resolvido</Button><Button size="sm" variant="ghost" onClick={()=>action.mutate({payment_id:p.id,action:'ignored'})}><X className="mr-2 h-4 w-4"/>Ignorar</Button></div></div>:<Button onClick={()=>gen.mutate(p)} disabled={gen.isPending}><Bot className="mr-2 h-4 w-4"/>{gen.isPending?<><Loader2 className="mr-2 h-4 w-4 animate-spin"/>Gerando...</>:'Gerar abordagem com IA'}</Button>}</CardContent></Card>})}</div></div></AdminLayout>
}
