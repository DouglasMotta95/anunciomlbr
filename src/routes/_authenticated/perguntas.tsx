...
function QuestionCard({ question, answerFn, suggestFn, afterSend }: { question: MlQuestion; answerFn: (args: any) => Promise<any>; suggestFn: (args: any) => Promise<any>; afterSend: () => void }) {
  ...
  return <Card><CardContent className="space-y-3 pt-5"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="font-semibold">{question.item_id}</p><p className="mt-1 text-xs text-muted-foreground">{formatDateTime(question.date_created)}</p></div><Badge variant={answered?"default":"secondary"}>{answered?"Respondida":"Aguardando resposta"}</Badge></div><div className="rounded-xl bg-muted/50 p-3"><p className="text-sm">{question.text}</p></div><Textarea rows={3} disabled={answered} value={text} onChange={(e)=>setText(e.target.value)} placeholder="Digite sua resposta ou peça uma sugestão à IA…" maxLength={2000}/><div className="flex flex-wrap justify-between gap-2"><span className="text-xs text-muted-foreground">{text.length}/2000 caracteres</span>{!answered&&<div className="flex gap-2"><Button variant="outline" size="sm" onClick={()=>suggest.mutate()} disabled={suggest.isPending}>{suggest.isPending?<Loader2 className="mr-2 h-4 w-4 animate-spin"/>:<Sparkles className="mr-2 h-4 w-4"/>}Sugerir com IA</Button><Button size="sm" onClick={()=>send.mutate()} disabled={send.isPending||!text.trim()}>{send.isPending?<Loader2 className="mr-2 h-4 w-4 animate-spin"/>:<Send className="mr-2 h-4 w-4"/>}Enviar resposta</Button></div>}</div></CardContent></Card>;
}
