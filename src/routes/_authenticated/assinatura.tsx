...
import { AlertTriangle, BadgeCheck, Brain, CalendarDays, CreditCard, PackagePlus, RefreshCcw, ShieldCheck, TrendingUp } from "lucide-react";
    ...
    <AppShell title="Central da assinatura" description="Plano, consumo, créditos, tentativas de pagamento e próximos passos em um único lugar.">
      <div className="space-y-5">
        <section className="grid gap-4 lg:grid-cols-[1.2fr_.8fr]">
          <Card className="border-primary/25">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2"><Button asChild><Link to="/licenca"><TrendingUp className="mr-2 h-4 w-4" />Ver upgrade</Link></Button>{data.license && <Button asChild variant="outline"><Link to="/creditos"><PackagePlus className="mr-2 h-4 w-4" />Comprar anúncios extras</Link></Button>}</div>
            </CardContent>
          </Card>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Card><CardHeader><CardTitle className="text-base">Precisa de mais capacidade?</CardTitle></CardHeader><CardContent><p className="text-sm text-muted-foreground">Você escolhe: subir de plano ou comprar somente a quantidade extra de anúncios que precisa.</p><div className="mt-4 flex flex-wrap gap-2"><Button asChild><Link to="/creditos">Comprar extras</Link></Button><Button asChild variant="outline"><Link to="/licenca">Comparar planos</Link></Button></div></CardContent></Card>
        </div>
      </div>
    </AppShell>
  );
